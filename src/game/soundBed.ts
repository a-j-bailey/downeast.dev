...
export function soundProfile(mood: WeatherMood): SoundProfile {
  switch (mood) {
    case "clearDay":
      return { waterGain: 0.11, horn: false, hornMinMs: 0, hornMaxMs: 0 };
    case "overcast":
      return { waterGain: 0.1, horn: true, hornMinMs: 55_000, hornMaxMs: 130_000 };
    case "rain":
      return { waterGain: 0.14, horn: false, hornMinMs: 0, hornMaxMs: 0 };
    case "fog":
      return { waterGain: 0.08, horn: true, hornMinMs: 35_000, hornMaxMs: 85_000 };
    case "night":
      return { waterGain: 0.07, horn: true, hornMinMs: 50_000, hornMaxMs: 120_000 };
    default: {
...
export class SoundBed {
  ...
  private decodeAndStart(): Promise<void> {
    ...
    this.decodeWait = this.decodeBuffers()
      .then(() => {
        this.ensureWater();
        this.applyMoodGains();
        this.sync();
  ...
  private applyMoodGains(): void {
    ...
    this.waterGain.gain.linearRampToValueAtTime(profile.waterGain, now + 1.15);
  }
  ...
  private armHorn(): void {
    ...
    const span = Math.max(0, profile.hornMaxMs - profile.hornMinMs);
    const delay = profile.hornMinMs + Math.random() * span;
